...
const SECTION_LEVEL_RE = /^(?<Ordinal>First|Second|Third|Fourth|Fifth|Sixth|Seventh|Eighth|Ninth|Tenth|Eleventh|Twelfth|Thirteenth|\d{1,2}(?:st|nd|rd|th))-Level\s+(?:(?<Class>Priest|Wizard|Mage|Cleric|Druid|Defiler|Preserver)\s+)?(?:Spells?|Psionic)/i;
